...
model.add(tf.keras.layers.Dense(22,  activation = tf.nn.softmax)) #stimmt so noch nicht mit 21 Fällen 
...
#y_train = np.arange(22) / np.max(np.arange(22))
#y_train = np.arange(22) / np.max(np.arange(22))
